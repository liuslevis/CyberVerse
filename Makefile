...
server:
	...
	@if [ -x /usr/lib/go-1.25/bin/go ]; then export PATH=/usr/lib/go-1.25/bin:$$PATH; fi; set -a; [ -f ./.env ] && . ./.env; set +a; cd server && go run -tags livekit ./cmd/cyberverse-server/ --config ../cyberverse_config.yaml
